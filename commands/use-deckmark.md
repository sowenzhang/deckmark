...
7. If screenshot tooling is available, capture the current build after it finishes and save all settled-slide captures and meaningful before/after motion states under `.deckmark/artifacts/`. Dispatch the critic prompt once to a different-model read-only reviewer when supported, then call `audit_deck` with the structured result, returned `run_id`, returned `build_hash` as `prepared_build_hash`, and `iteration: 1`. Fix blockers, rebuild, capture a fresh static set, and repeat for at most three passes with the same `run_id`. Advisory mode may continue with clearly reported limitations; blocking mode requires rendered evidence and an honestly reported independent reviewer.
